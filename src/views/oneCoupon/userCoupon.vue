<template>
  <div class="user-coupon"
    ...
    <van-swipe class="my-swipe"
               v-if="config.switchControl==1">
      <van-swipe-item @click="selectImg(config.skipUrl)"><img :src="config.bannerUrl" /></van-swipe-item>
    </van-swipe>
    <!-- 新人券 -->
...
import { add, getUrlParameter } from "@/utils/index.js";
...
export default {
  ...
  created () {
    ...
    this.id = this.$route.query.couponId || getUrlParameter("couponId");
    this.shareConfig.linkUrl = `${appUrl}oneCoupon?id=${this.id}`;
    this.init();
  },
  mounted () {
    if (window.sessionStorage.getItem('HPC-TOKEN')) {
      this.$store.commit("setAppToken", window.sessionStorage.getItem('HPC-TOKEN'));
    }
    this.getCoupon();
  ...
  methods: {
    ...
    getConfig () {
      this.$api.oneCoupon.getReceive({couponId: this.id}).then(res => {
        if (res) {
          this.config.bgColor = res.backValue;
          this.config.switchControl = res.switchControl;
          this.config.bannerUrl = res.bannerUrl;
          this.config.skipUrl = res.skipUrl;
          this.config.skipSet = res.skipSet;
          this.bg2 = res.backgroudUrl;
    ...
    getCoupon () {
      this.countPrict = 0;
      this.$api.oneCoupon.getCoupon(this.query).then((res) => {
        if (!!res) {
    ...
    selectImg (v) {
      if (!!v && this.config.skipSet == 0) {
        window.location.href = v;
      }
    },
    async couponData () {
      this.$api.oneCoupon.getCouponList({couponId:this.query.couponId,userId:this.query.userId}).then((res) => {
        if (res.couponUserId!=0) {
          this.couponAllList = [res];
        this.newCouponList =  [res].filter(v => v.newUserFlag === 1);
        this.couponList =  [res].filter(v => v.newUserFlag !== 1);
        }
      });
    },
    download () {
