export const ruler = {
  ...
  initPlugin: function (params) {
    var initParams = {
      ...
      maxScale: params.textArray
        ? (params.textArray.length - 1) * 10
        : params.maxScale || 200,
      startValue: params.textArray
        ? params.startValue * 10
        : params.startValue || params.startValue,
      region: params.textArray
        ? (params.region[1] = (params.textArray.length - 1) * 10)
        : params.region || false,
      background: params.background || false,
      color: params.color || false,
      markColor: params.markColor || '#FFCC33',
      markHeight: params.markHeight || 0.56,
    ...
    //最大刻度的小值是20(自己修改的)
    initParams.maxScale = initParams.maxScale < 20 ? 20 : initParams.maxScale
    ...
    function drawRuler(count) {
      ...
      for (var i = 0; i <= initParams.maxScale; i++) {
        ...
        if (initParams.textArray.length == 0) {
          cxt.lineTo(
      ...
      scaleValueList.forEach(function (num, i) {
        if (initParams.textArray.length != 0) {
          var text = initParams.textArray[parseInt(num / 10)]
          cxt.fillText(
            text,
            division * i * 10 - count * division,
            Math.floor(initParams.height * 0.6),
          )
        } else {
          cxt.fillText(
            num.toString(),
    ...
    function moveEvent() {
      ...
      if (initParams.isConstant) {
        if (initParams.textArray.length !== 0) {
          params.success && params.success(Math.round(count / 10))
        } else {
