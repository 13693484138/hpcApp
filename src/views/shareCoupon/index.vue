        ...
        if (this.code != this.validate) {
          ...
          this.bindMobile().then(() => {
            ...
            let timer = setTimeout(() => {
              this.$toast.clear();
              this.$router.push({ path: '/userCoupon', query: { userType: this.userType } });
              clearTimeout(timer);
              timer = null;
              this.isShow = false;
            }, 1800)
    ...
    userInit () {
      ...
      if (getUrlParameter("unionId") != undefined) {
        ...
        if (ua.match(/MicroMessenger/i) == "micromessenger") {
          wxAuth(`${appUrl}shareCoupon`); //进行微信授权
        }
        // app内部无返回手机号 产品讲app不会跳转抢券页面
        // if (isInApp()) {
        //   let loginType = getAppInfo() != null ? (getAppInfo().sign ? true : false) : false;
        //   if (loginType===false) {
